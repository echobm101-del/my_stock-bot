...
def get_news_sentiment_llm(name, stock_context={}):
    ...
    if res_data:
        try:
            ...
            return {"score": js.get('score', 0), "supply_score": js.get('supply_score', 0), "headline": js.get('summary', "분석 완료"), "opinion": js.get('opinion', "중립"), "risk": js.get('risk', "없음"), "catalyst": js.get('catalyst', ""), "raw_news": unique_news, "method": "ai", "dart_text": dart}
        except: pass
...
def get_ai_recommended_stocks(keyword):
    prompt = f"'{keyword}' 관련 한국 주식 5개를 추천해주세요. JSON 형식: [{{'name':'삼성전자', 'code':'005930', 'relation':'대장주'}}]"
