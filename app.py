...
import random
from dateutil import parser as date_parser # 날짜 파싱 보조
...
# --- [2. 시각화 및 렌더링 함수] ---
# (기존 시각화 함수들은 변경사항 없이 유지 - 분량상 핵심 로직 위주)
                ...
                with col2:
                    ...
                    if res.get('dart_disclosures'):
                        ...
                        for d in res['dart_disclosures']:
                            st.markdown(f"<div class='dart-box'>{d['report_nm']} <span style='color:#888'>{d['rcept_dt']}</span></div>", unsafe_allow_html=True)
        ...
        for res in results:
            ...
            with st.expander(f"📊 상세 분석: {res['name']}"):
                if res['news']['method'] == "standby":
                    ...
                    st.success(f"AI 코멘트: {res['news']['headline']}")
...
with tab3:
    ...
    if not wl_items: st.info("관심 종목이 없습니다.")
        ...
        for res in results:
            if res['code'] in st.session_state['ai_cache']: res['news'] = st.session_state['ai_cache'][res['code']]
            st.markdown(create_watchlist_card_html(res), unsafe_allow_html=True)
            with st.expander(f"▼ 상세 보기"):
                if st.button(f"🚀 AI 분석 ({res['name']})", key=f"wl_ai_{res['code']}"):
                    ai_res = get_news_sentiment_llm(res['name'], res['ai_context'])
                    st.session_state['ai_cache'][res['code']] = ai_res
                    st.rerun()
                if 'headline' in res['news']: st.info(res['news']['headline'])
                render_investor_chart(res['investor_trend'])
...
with st.sidebar:
    st.write("### ⚙️ 기능 메뉴")
    if USER_DART_KEY and dart: st.success("✅ DART 연결됨")
    else: st.warning("⚠️ DART 키 확인 필요")
