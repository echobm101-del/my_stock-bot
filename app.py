...
# -------------------------------------------------------------------------
# [핵심] API 호출 (자동 탐색: 사용 가능한 모델을 조회 후 사용)
# -------------------------------------------------------------------------
def get_valid_model_name(api_key):
    """
    내 API 키로 사용 가능한 모델 목록을 조회하여 첫 번째로 발견된 유효한 모델 이름을 반환합니다.
    """
...
def call_gemini_dynamic(prompt):
    ...
    # [설정] temperature 0.0 설정 (창의력 제거, 팩트 위주)
    payload = {
